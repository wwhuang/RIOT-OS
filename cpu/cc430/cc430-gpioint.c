...
interrupt(PORT1_VECTOR) __attribute__((naked)) port1_isr(void)
{
    ...
    if (debounce_flags[0] & p1ifg) {
        /* check if bouncing */
        diff = xtimer_usec_from_ticks(xtimer_now()) - debounce_time[0][ifg_num];

        if (diff > DEBOUNCE_TIMEOUT) {
            debounce_time[0][ifg_num] = xtimer_usec_from_ticks(xtimer_now());
...
interrupt(PORT2_VECTOR) __attribute__((naked)) port2_isr(void)
{
    ...
    if (debounce_flags[1] & p2ifg) {
        /* check if bouncing */
        diff = xtimer_usec_from_ticks(xtimer_now()) - debounce_time[1][ifg_num];

        if (diff > DEBOUNCE_TIMEOUT) {
            debounce_time[1][ifg_num] = xtimer_usec_from_ticks(xtimer_now());
            c1++;
