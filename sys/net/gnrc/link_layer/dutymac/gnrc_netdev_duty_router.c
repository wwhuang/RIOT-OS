...
void msg_queue_send(msg_t* msg_queue, uint16_t dst_l2addr, gnrc_netdev_t* gnrc_dutymac_netdev) {
        ...
        switch (msg.type) {
			case GNRC_NETDEV_DUTYCYCLE_MSG_TYPE_SND:
				...
				if (pending_num && !radio_busy) {
					msg_queue_send(pkt_queue, *((uint16_t*)msg.content.ptr), gnrc_dutymac_netdev);
				}
				break;
			case GNRC_NETDEV_DUTYCYCLE_MSG_TYPE_REMOVE_QUEUE:
				...
				if (pending_num && !radio_busy && recent_dst_l2addr != 0xffff) {
					...
					msg_queue_send(pkt_queue, recent_dst_l2addr, gnrc_dutymac_netdev);
				} else if (!pending_num) {
					bool pending = false;
