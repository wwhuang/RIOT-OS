...
{
    *target = xtimer_usec_from_ticks64(xtimer_now64()) + (ms * MS_IN_USEC);
}
...
{
    uint64_t tm = fib_sr->sr_lifetime - xtimer_usec_from_ticks64(xtimer_now64());
    /* check if the lifetime expired */
    if ((int64_t)tm < 0) {
                     ...
                     uint32_t *sr_flags, uint32_t *sr_lifetime)
{
    ...
    *sr_lifetime = fib_sr->sr_lifetime - xtimer_usec_from_ticks64(xtimer_now64());
